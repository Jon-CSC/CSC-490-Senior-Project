...
public class BoardLogic {
    ...
    /**
     * Resets all game-specific fields, including board configuration, to their
     * initial values.
     *
     * @see Board#reset()
     */
    ...
    /**
     * Identifies board position corresponding to a given mouse click
     * coordinates.
     *
     * @param mouseX
     * @param mouseY
     * @return board position as BoardPos object if coordinates in range, null
     * otherwise
     */
    ...
    public List<BoardPos> getMoves(BoardPos from) {
        ...
        if (result.isEmpty() && !board.get(from).isEmpty()) {
            if (board.get(from).isCrown()) {
                ...
                for (int shift : shifts) { // add adjacent empty positions
    ...
    /**
     * Checks relevant board positions to see if some pieces can be promoted to
     * crown, does so if there are.
     */
